...
Validate material decisions with campaign context and experiments where possible. Interaction output describes the selected model and data period. It does not prove that changing one channel will cause the other channel's performance to change. <br /><br />Before shifting significant budget based on an interaction finding, confirm it with a test, such as a geo experiment, that measures the real-world effect.
