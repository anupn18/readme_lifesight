...
excerpt: Incrementality at the speed of Performance Marketing
deprecated: false
hidden: false
metadata:
  title: ''
...
Incremental Revenue (iRevenue) =
    0.5 * [ Incrementality Factor (from MMM or Experiments) * Platform-Reported Revenue (pRevenue) ]
  + 0.5 * [ miROAS-based estimate ]
```

**Component 1 - Incrementality Factor × pRevenue (50% weight)**<br />This is the original attribution-calibrated approach. The incrementality factor (derived from MMM or experiments) is applied to platform-reported revenue (pRevenue) to arrive at an incremental revenue (iRevenue) estimate. This grounds the number in the channel's tracked, granular performance.

**Component 2 - Marginal iROAS (miROAS) (50% weight)**<br />The remaining 50% weightage comes from miROAS, which is computed at the tactic level and then scaled down to the campaign level (and further to ad set level).

> 📘 Underlying assumption: Campaigns and ad sets are assumed to saturate at the same rate as the tactic they belong to. This allows the tactic-level marginal return (miROAS) to be distributed down to the campaign and ad set levels proportionally.

<br />

By blending the attribution-calibrated component with the marginal-return component, the final incremental revenue reflects both the historically tracked contribution of a channel and its expected marginal performance at the current spend level.

📘 Note: The earlier methodology relied solely on the formula Incremental Conversions = Incrementality Factor × Platform-Reported numbers (pConversions). This is now Component 1 (carrying 50% of the weight); the miROAS-based marginal-return component (Component 2) makes up the other 50%.
...
1. **Incremental ROAS (iROAS)**:<br />This is derived from Marketing Mix Modeling. It represents the additional return generated for each dollar spent on a particular marketing activity, above what would have occurred without that activity.

2. **Platform Reported ROAS (pROAS)**:<br />This is the return on ad spend as reported by the advertising platform itself. It often includes all conversions that occurred after exposure to an ad, regardless of whether the ad truly caused the conversion.

3. **Incrementality Factor**:<br />By dividing iROAS and pROAS, we get a measure of the true incremental revenue generated by the marketing activity. This incrementality factor is helpful because it reveals the true additional value generated by a marketing channel beyond platform-reported figures, guiding more effective budget allocation and strategic decisions for genuine growth.
...
4. **Platform Conversions**:<br />The conversion numbers reported by the ad platform; this includes metrics like Revenue, Orders, Leads, Signups, Registrations, Add to Carts, App Installs and Custom Conversions.
...
1. **Data Collection**:<br />Gather comprehensive data from all marketing channels, including spend, impressions, clicks, and conversions. This also includes broader business data like sales, pricing, and external factors (e.g., seasonality, competitor activities).
   > 📘 Lifesight currently supports two anchor methodologies for causal attribution - Platform reported signals and GA4 reported signals

2. **Marketing Mix Modeling**:<br />Develop and run an MMM model to determine the incremental impact of marketing activities at a high level. This generates the iROAS values.

3. **Multi-Touch Attribution**:<br />Implement an MTA solution to capture granular, user-level interactions across marketing touchpoints.

4. **Data Integration**:<br />Combine the MMM and MTA data, ensuring alignment in metrics and time periods.

5. **Causal Model Development**:<br />Create a model that applies the causal attribution formula, integrating the iROAS from MMM with the detailed attribution data from MTA.

6. **Validation and Testing**:<br />Validate the model results against historical data and through controlled experiments where possible.

7. **Ongoing Calibration**:<br />Regularly update the model with new data and refine it based on changing market conditions and business objectives.
